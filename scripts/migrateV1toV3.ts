...
function validProfile(p: OLD_MONGO_Profile) {
  if (!Object.keys(p.appOpens).length) {
    log.error("No app opens detected", {
      p: p.user,
      usage: {
        appOpens: p.appOpens,
        matches: p.matches,
...
async function main() {
  ...
  try {
    for (const file of files) {
      ...
      for (let j = 0; j < mongoProfiles.length; j += BATCH_SIZE) {
        const batch = mongoProfiles.slice(j, j + BATCH_SIZE);
        ...
        await Promise.all(
          batch.map(async (profile) => {
            ...
            try {
              ...
              const tinderJson = mapMongoProfileToOriginalTinderJson(profile);
              const [jsonDataIsValid, jsonErrors] =
              ...
              const durationInSeconds = ((endTime - startTime) / 1000).toFixed(
                2,
              );
...
async function loadFile(path: string) {
  console.log("Starting parsing", path);
